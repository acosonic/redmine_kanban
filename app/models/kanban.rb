class Kanban
  ...
  def get_users
    if @user
      ...
      if role_id
        query_conditions = "#{MemberRole.table_name}.role_id = ?" % role_id
        query_conditions << "#{MemberRole.table_name}.member_id = #{Member.table_name}.id"
        query_conditions << "#{Member.table_name}.user_id = #{User.table_name}.id"
        @users = User.active.all(:conditions => query_conditions,
                          :select => "users.*",
